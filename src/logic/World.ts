...
export class World implements IGlobalWorld {
  private objects: Dictionary<number, IWorldObject> = new Dictionary<
    number,
    IWorldObject
  >()
  ...
  public getPlayerPosition(): Vector3 {
    return this.objects.get(1)!.getPosition()
  }
  ...
  private checkObjectInSight(
    ...
    // 캐릭터에서 대상까지의 벡터
    var objectVector = target
      .getPosition()
      .subtract(eyeSight.getSightPosition())
  ...
  public checkObjectsInSight(
    ...
    for (const obj of this.getIterWorldObjects()) {
      ...
      if (this.checkObjectInSight(obj, eyeSight)) {
        objectsInSight.push([
          obj,
          obj.getPosition().distanceSquared(eyeSight.getSightPosition()),
        ])
      }
    }
